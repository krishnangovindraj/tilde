...
def run_task(config: TildeConfig):
    ...
    if test_examples_collection is not None:  
        ...
        print("EXP: UNSUPERVISED")      
        test_examples = None
        all_examples = [e for e in examples]
    
    # from random import sample as random_sample
    # temp_sample = random_sample(all_examples, int(len(all_examples) * 0.1))
    # for ts in temp_sample:
    #     print( str(ts.classification_term) )
    # quit()
    ...
    for _trial_i in range(0, 1):
        ...
        target_examples = test_examples if test_examples is not None else examples
        sorted_examples = sorted([ (model.predict(e),e) for e in target_examples ], key=lambda t: t[0], reverse=True)
        for score,e in sorted_examples: # [:10]: # [:10]:
            print(round(score,2), str(e.classification_term))
            print("\n".join(["\t%d: %s"%(r[1],r[0]) for r in model.explain(e, 2)]))
