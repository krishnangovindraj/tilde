...
class IsolationForestSimpleSplitter(GenerateAndSampleSplitter):
    ...
    def _sample_tests(self, all_tests: List[TILDEQuery], examples: List[Example]) -> List[TILDEQuery]:
        ...
        while len(all_tests)>0 and tests_tried != self.n_retry_before_giveup:
            ...
            if True: # len(examples_satisfying_test) > 0: # examples_not_satisfying test don't even have new variables introduced :/ 
