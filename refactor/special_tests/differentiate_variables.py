...
class DifferentiateVariablesTest(SpecialTest):

    TEST_FUNCTOR_PREFIX = "tilde__diffvars_"
    ARG_MODES = ('+', '+')
    ...
    def setup(self, prediction_goal_handler, language, examples, bg_sp):
            ...
            for d in list(e.data) + [e.classification_term]:
                if (d.functor, d.arity) in value_locations:
                    for i in value_locations[(d.functor, d.arity)]:
                        example_values[e].append(d.args[i])
        ...
        bg_values = list(bg_values)
        # All work done here:
        self._saturate_examples(examples, example_values, bg_values)
    ...
    def _saturate_examples(self, examples: List[Example], example_values, bg_values):
        for e in examples:
            visible_values = self.all_values # example_values[e] + bg_values
            new_facts = [Term(self.test_functor, g, h) for g in visible_values for h in visible_values if g!=h]
